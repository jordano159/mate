json.set! :data do
  json.array! @kids do |kid|
    ...
    json.url  "
              ...
              #{link_to '<i class="fas fa-edit"></i>'.html_safe, edit_kid_path(kid), style: 'color: grey', class:'mx-2' if current_staff.admin? || current_staff.vip?}
              #{link_to "<i class='far fa-trash-alt'></i>".html_safe,  kid, method: :delete, data: { confirm: 'את/ה בטוח/ה?' },style: "color: red" if current_staff.admin? || current_staff.vip?}
              "
  end
end
